...
- Agent 系统提示：`agents/<name>.md`，在 `plugin.json` 的 `agents` 字段注册
- Skill 实现：`skills/<name>/SKILL.md`，在 `plugin.json` 的 `skills` 字段注册
- `plugin.json` 的 `commands` 为空对象 `{}` — skills 注册会自动生成 `/<plugin-name>:<skill-name>` 格式的斜杠命令
...
```powershell
Get-Content plugin.json | ConvertFrom-Json | Out-Null
```
